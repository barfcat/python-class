...
#all neighbors output easier to use
cdp_neighbors = [filter(None, sw1_show_cdp_neighbors.split("\n")), filter(None, r1_show_cdp_neighbors.split("\n")), filter(None, r2_show_cdp_neighbors.split("\n")), filter(None, r3_show_cdp_neighbors.split("\n")), filter(None, r4_show_cdp_neighbors.split("\n")), filter(None, r5_show_cdp_neighbors.split("\n"))]
...
#all details output easier to use
...
for num in range(len(cdp_neighbors)):
    for key in network_devices.keys():
        network_devices[key]['adjacent_devices'] = []

for num in range(len(cdp_neighbors)):
    for key in network_devices.keys():
        if key in cdp_neighbors[num][0]:
            for line in cdp_neighbors[num][4:]:
                network_devices[key]['adjacent_devices'].append(line.split(' ')[0])

#test our code
pprint.pprint( network_devices)

##add information to individual device dictionaries
#for variable in dir():
#    for key in network_devices.keys():
#        if key.lower() in variable:
#            if 'detail' in variable:
